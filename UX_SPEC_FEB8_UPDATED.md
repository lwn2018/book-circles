...
### Post-borrow UI behavior — IMPORTANT

When a user taps Borrow and confirms, do NOT immediately re-sort the book list. The book just vanishing to the bottom of the page (where borrowed books live in the sort order) feels like something broke.

Instead:

1. **Show a confirmation toast** at the top or bottom of the screen: "You're borrowing [Book Title]! [Owner name] has been notified." Toast auto-dismisses after 3-4 seconds.
2. **Update the card in place.** The card stays exactly where it is in the list. Swap the green "Available" badge to yellow "With [your name]," reduce cover opacity to ~70%, and replace the Borrow button with the status indicator. Let the user see the state change happen on the card they just tapped.
3. **Defer the re-sort.** The next time the user leaves the circle and comes back, the normal sort order applies and the book appears in the borrowed section. But during this session, the card stays put.

This same pattern applies when tapping Request on a borrowed book — the card stays in place, badge updates to show the user's queue position, and re-sort happens on next visit.
