...
export default async function BookDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusLabel = book.status === 'available' ? 'Available' 
    : book.status === 'borrowed' ? 'Currently Reading' 
    : book.status === 'in_transit' ? 'In Transit' 
    : 'Off Shelf'
  ...
  return (
    <div className="min-h-screen bg-[#121212] px-4 py-6 pb-32">
      ...
      <div className="bg-[#1E293B] rounded-xl p-6 mb-6 flex justify-center">
        <div className="w-40 h-56 rounded-lg overflow-hidden shadow-xl">
      ...
      <div className="bg-[#1E293B] rounded-xl p-4 mb-6">
        <div className="grid grid-cols-2 gap-4">
          {book.genres && book.genres.length > 0 && (
            <div>
              <p className="text-xs text-[#55B2DE] mb-1">Genre</p>
              <p className="text-white">{Array.isArray(book.genres) ? book.genres.join(', ') : book.genres}</p>
            </div>
          )}
          {publishYear && (
            <div>
              <p className="text-xs text-[#55B2DE] mb-1">Published</p>
              <p className="text-white">{publishYear}</p>
            </div>
          )}
          {book.page_count && (
            <div>
              <p className="text-xs text-[#55B2DE] mb-1">Pages</p>
              <p className="text-white">{book.page_count}</p>
            </div>
          )}
          {book.language && (
            <div>
              <p className="text-xs text-[#55B2DE] mb-1">Language</p>
              <p className="text-white capitalize">{book.language}</p>
            </div>
          )}
        </div>
